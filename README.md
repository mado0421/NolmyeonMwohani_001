...
#### 2022-09-07
적 캐릭터를 생성하고 관리할 매니저 오브젝트를 만들어야 한다. 적 캐릭터는 플레이어 캐릭터를 향해 계속 이동하고, 충분히 가까워지면 근접 공격으로 피해를 준다.

미리 적 캐릭터들을 생성해둘 풀을 만든다.  적 캐릭터는 프리팹으로 변환한 뒤, 에너미풀에 등록한다.
적 캐릭터들은 화면 밖에서 생성된다.
```C#
// 일정 시간마다 화면 밖에 소환한다.
...
```
적 캐릭터가 죽었을 때, 해당 오브젝트를 재사용하면서 캐릭터의 hp가 초기화되지 않아 바로 죽는 문제가 있었다. OnEnable()에서 초기화시켜주는 코드를 Character 클래스에 추가하여 해결하였다.

근접 공격은 어떤 방식으로 할 것인가? 간단하게 캐릭터 앞에 충돌체를 두고 충돌하면 데미지를 주는 식으로 해도 될 것이다. 또는, 충분히 가까워졌을 때, 공격할 대상 캐릭터에게 피해를 주고 쿨타임을 돌려도 될 것이다.

후자는 충돌체가 없어도 되겠구나. 후자가 괜찮을  것 같다. 적 캐릭터는 원래 플레이어 캐릭터를 목표로 하여 이동하기 때문에 플레이어 캐릭터의 transform을 갖고 있는다. (플레이어 캐릭터를 전역 오브젝트로 생성하고 인스턴스를 받아오는 것도 괜찮을 것 같다.) 플레이어 캐릭터는 근접 공격을 사용할 계획이 없다. 있다 하더라도 범위공격이 될 것이다.
